...
def create_encoder_model(input_shape):
    ...
    encoded = Conv2D(filters=3, kernel_size=(1,1), strides=(1,1), padding='same')(conv_relu1_concat)
    
    # pool1 = MaxPooling2D((2, 2))(conv1)
    
    # Create the model
    model = Model(inputs=[cover_img, hidden_img], outputs=encoded)
...
def create_decoder_model(input_shape):
    # Define the input layers for both images
    encoded_img = Input(shape=input_shape)
    
    # Apply conv-BN-Relu to encoded image (7 blocks in original paper)
    conv_relu1 = conv_bn_relu_block(encoded_img, filters=64, kernel_size=(3, 3))
    ...
    # Skip layer: Concatenate the conv-BN-relu results
    concat1 = Concatenate()([conv_relu5, encoded_img])
    conv_relu1_concat = conv_bn_relu_block(concat1, filters=64, kernel_size=(3, 3))
    ...
    # Create the model
    model = Model(inputs=[encoded_img], outputs=decoded)
